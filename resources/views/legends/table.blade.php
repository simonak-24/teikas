...
<table>
    ...
    <tr>
        ...
        @if(!($item instanceof App\Models\Collector))<th><div class="sort-cell"><div>{{ __('resources.legend_collector') }}</div>@if(gettype($item) != 'string')<div><a id="cl" class="sort-arrow" href="#" onclick="submitSort('cl')">{{ isset($sort['collector']) ? urldecode($sort['collector']) : "⭥" }}</a></div>@endif</div></th>@endif
        @if(!($item instanceof App\Models\Narrator))<th><div class="sort-cell"><div>{{ __('resources.legend_narrator') }}</div>@if(gettype($item) != 'string')<div><a id="nr" class="sort-arrow"  href="#" onclick="submitSort('nr')">{{ isset($sort['narrator']) ? urldecode($sort['narrator']) : "⭥" }}</a></div>@endif</div></th>@endif
        @if(!($item instanceof App\Models\Place))<th><div class="sort-cell"><div>{{ __('resources.legend_place') }}</div>@if(gettype($item) != 'string')<div><a id="pl" class="sort-arrow"  href="#" onclick="submitSort('pl')">{{ isset($sort['place']) ? urldecode($sort['place']) : "⭥" }}</a></div>@endif</div></th>@endif
    </tr>
    @if(!isset($search))
    <tr>
