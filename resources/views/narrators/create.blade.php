...
@section('content')
    ...
    <form action="{{ route('narrators.store') }}" method="POST">
        ...
        <table>
            ...
            <tr>
                <td><b><label for="external-id">{{ __('resources.external-link-humma') }}: </label></b></td>
                <td><input disabled type="number" id="external_id" name="external_id" value="{{ old('external_id', $narrator->external_id) }}"></td>
            </tr>
        </table>
        <br>
