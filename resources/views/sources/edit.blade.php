...
@section('popup')
    <div id="resource-delete" class="delete-popup">
        ...
        @if($source->legends()->count() > 1)
        <p>{{ __('resources.source_delete-multiple', ['count' => $source->legends()->count()]) }}</p>
        @elseif($source->legends()->count() > 0)
        <p>{{ __('resources.source_delete-single') }}</p>
        @endif
